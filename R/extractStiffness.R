newStiffness=function(case,r,approachLength=.1,contactLength=.1,searchWidth=.2,maxF=.5,weight=4,zPos="zSensr",force="force"){
    ...
    for(cpIndex in cpToCheck){
        print(paste("checking index",iteration,"of",length(cpToCheck)))
...
#        allFits=c(allFits,fits)
    ...
    fitPlot=ggplot(correctedData,aes(x=zPos,y=F))+geom_path(color="blue")+geom_path(data=modelCurve,color="red")
    plot(fitPlot)

    fitdata=data.frame(contactIndex=contactPoint$cpIndex,contactPos=contactPoint$zSensr,contactForce=contactPoint$force,residual=lowestError,EStar=coef(bestFit$nonLinear)["EStar"],converged=bestFit$nonLinear$convInfo$isConv)
    ...
    fit=list(fit=fitData,curves=curves)
